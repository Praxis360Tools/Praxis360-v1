...
Every final textual reply to the user must be structured for complete one-action copy.
...
Place the entire response inside this single code block.

Do not place any sentence before the opening fence.

Do not place any sentence after the closing fence.

Do not create a second code block inside the main block.

Represent commands, file paths, and technical excerpts by indentation within the main block.
...
The user must be able to copy the entire response in one action.

**Exceptions**

Interactive buttons generated by Visual Studio or Copilot itself are excluded from this rule.
